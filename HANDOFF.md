...
## 50. The benchmark page, and the aider execution bug named upstream

### `benchmark.html` - the human and Timema results as a reviewable page

Published at
`https://toki-bio.github.io/SINE-discriminator/benchmark.html`, linked from the
top of the main page. All 119 candidates from both independent annotations, with
score at 50 bp and at 400 bp, delta, decay call, flags (hover for full text), and
links opening each alignment in the MSA viewer **at both flank widths** - 227
alignment files were added to `site/alignments/`, with the 400 bp geometry
suffixed `.ext400.aln.fa`.

Filters deliberately surface what needs review rather than what passes:
`below 50`, `flagged`, `moved by 5+`, `not isolated`. Rows scoring under 50 are
tinted. Built from `bench_page.json`, regenerated by re-scoring both corpora.

The page states plainly, next to the Timema table, that the `matched`/`noisy`
labels measure **subfamily assignment leak** and not sine/not-sine - so a
`noisy` candidate scoring 100 is not a disagreement (§39, §47). Without that
note the table looks like the tool failing, which is the wrong conclusion.

Verified in headless Chrome before publishing: 64 + 55 rows, filters live, 227
links, no console errors, no horizontal overflow.

### Why the model never executed anything - upstream bug, now named

`--yes-always` does not auto-confirm suggested shell commands, it **suppresses
them entirely**: they are neither run nor prompted for. That is
[Aider-AI/aider#3903](https://github.com/Aider-AI/aider/issues/3903), open since
2025-04-28, no fix. This harness always passes `--yes-always`, so **no suggested
shell command has ever run in any run of this harness** - which fully explains
the repeated "wrote correct code, produced no output" failures.

`--lint-cmd` / `--test-cmd` DO execute automatically (`--auto-lint` defaults
true) and are the documented workaround, with the bonus that failure output is
fed back so the model can self-correct in-run. But `--auto-test` is avoided here
for context reasons, so the primary fix stays: **the deterministic check script
runs the target itself.** Full write-up in `ORCHESTRATION-MANUAL.md` §16-17a.

### GLM, run properly

The task that had burned 8 runs producing nothing was re-cut as: a **skeleton
file with a single `NotImplementedError` body**, a docstring fixing every rule
and edge case, a short task file, a fresh progress file, and no large extra
files in context. Same model, same backend: **3.6k tokens sent, 2.0k received,
correct implementation on the first run.** The earlier failure was a
deliberation loop caused by leaving open questions in the spec - not a backend
problem.
