...
| teu SubFam-per-subfamily alignment | `GitHub` | `Toki-bio/Tal/main/teu/alignments/teu_subfam_input.aln.fa` | ok | HE POINTED ME HERE. 1206 rows = 6 blocks of 201 (200 chunks + consensus) at rows 1-201, 202-402, 403-603, 604-804, 805-1005, 1006-1206. Labels known by construction. TRAP: 200 distinct names each repeated 6x, so scoring by name mixes blocks |
| teu benchmark, names disambiguated | `KIT` | `/data/W/toki/SINE_disc/teu_curate/teu_sf_uniq.fa` + `teu_sf_truth.tsv` | ok | same file with original row index appended to each name, plus the chunk-to-subfamily truth table |
| teu genome-wide curation alignment | `KIT` | `/data/W/toki/SINE_disc/teu_curate/teu_curate.aln.fa` | ok | 606 rows, 600 genome-wide chunks + t1-t6 landmarks, built from run_20260427_130055 |
| Scorpion AnnoSINE runs (all 9) | `DRAGEN` | `/staging/tmp/scorpions/<code>/anno/Seed_SINE.fa` | ok | aeg 22, amu 35, bku 26, bxa 34, csc 37, cvi 15, efe 112, hta 33, oma 33 = 347 candidates; finished 2026-09-03 |
| Scorpion AnnoSINE runs (efe, bxa only) | `KIT` | `/data/V/toki/Genomes/Scorpions/<code>/anno/Seed_SINE.fa` | ok | the two largest genomes, run by `/data/V/toki/anno_kit.sh`; same counts as DRAGEN's independent runs |
